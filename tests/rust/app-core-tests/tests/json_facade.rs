...
use slan_app_core::{
    ...
    Network, NetworkMap, Node, Peer, RelayCity, RelayCluster, RelayConfig, RelayCountry,
...
impl PathManager for NoopPathManager {
    fn on_p2p_failed(&self, peer_node_id: &str, _reason: &str) -> Result<(), String> {
...
impl PathManager for FailingPathManager {
    fn on_p2p_failed(&self, _peer_node_id: &str, _reason: &str) -> Result<(), String> {
        Ok(())
    }

    fn on_p2p_recovered(&self, _peer_node_id: &str) -> Result<(), String> {
        Ok(())
    }
    ...
    fn send_transport_packet(&self, _packet: &[u8]) -> Result<(), String> {
        Err("timed out waiting for probe reply".to_string())
    }
...
impl PathManager for UnsupportedPathManager {
    ...
    fn send_transport_packet(&self, _packet: &[u8]) -> Result<(), String> {
        Err("unsupported active path selected for send".to_string())
    }
