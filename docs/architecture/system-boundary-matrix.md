...
| `server/service-biz` | 业务控制面，负责账号、网络资产、设备归属、业务权限和运营 API | WireGuard 路径规划、真实流量转发 |
| `server/opt-ui` | 统一运营/运维控制台，负责用户、设备、设备分组、网络、安全组、DNS 和节点管理 | 客户端控制面、数据面转发 |
| `server/server-wire` | 联网控制面，负责 peer 注册、runtime config、路径规划、relay/DERP ticket 签发 | 用户、组织、审计等业务域；商业计费已退役 |
