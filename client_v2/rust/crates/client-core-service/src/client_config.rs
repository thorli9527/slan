...
const KEY_DEVICE_PUBLIC_KEY: &str = "devicePublicKey";
...
pub(crate) fn load_device_public_key(device_id: &str) -> Result<Option<String>> {
    load_secret(device_id, KEY_DEVICE_PUBLIC_KEY)
}

pub(crate) fn store_device_public_key(device_id: &str, public_key: &str) -> Result<()> {
    store_secret(device_id, KEY_DEVICE_PUBLIC_KEY, &public_key.trim())
}
