...
- `postinstall` defensively stops any old launchd jobs and stale UI/service processes again, then runs `client-core-service --ensure-device-id` to create the UUID device ID only when it does not already exist. Upgrades preserve the existing device identity; a full uninstall/reset is the boundary that removes local state.
