...
rsync -az --delete \
  -e "$(printf '%q ' "${SSH_PREFIX[@]}" ssh "${SSH_OPTS[@]}")" \
  --exclude '.git/' \
  --exclude '.DS_Store' \
...
"${SSH_PREFIX[@]}" ssh "${SSH_OPTS[@]}" "$SSH_TARGET" \
  "cd '$REMOTE_DIR' && docker compose --env-file '$ENV_FILE' -f '$COMPOSE_FILE' up -d --build --remove-orphans"

echo "==> Remote compose status"
"${SSH_PREFIX[@]}" ssh "${SSH_OPTS[@]}" "$SSH_TARGET" \
  "cd '$REMOTE_DIR' && docker compose --env-file '$ENV_FILE' -f '$COMPOSE_FILE' ps"
