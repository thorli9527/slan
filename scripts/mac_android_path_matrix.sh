...
BIZ_URL="${SLAN_BIZ_URL:-http://47.245.40.231:28080}"
ANDROID_BIZ_URL="${SLAN_ANDROID_BIZ_URL:-$BIZ_URL}"
SERVER_HOST="${SLAN_SERVER_HOST:-47.245.40.231}"
...
run_udp_relay() {
  ...
  run_socket_check udp-relay \
    ...
    SLAN_EXPECT_ANDROID_RELAY_FRAMES_SENT_MIN="${SLAN_EXPECT_RELAY_FRAMES_SENT_MIN:-1}"
}
...
run_tcp_relay() {
  ...
  run_socket_check tcp-relay \
    ...
    SLAN_EXPECT_ANDROID_RELAY_TCP_FRAMES_RECEIVED_MIN="${SLAN_EXPECT_RELAY_TCP_FRAMES_RECEIVED_MIN:-1}"
}
