...
if [[ -n "${SLAN_MULTI_ANDROID_CONTROL_BASE_URL:-}" ]]; then
  ANDROID_CONTROL_BASE_URL="$SLAN_MULTI_ANDROID_CONTROL_BASE_URL"
elif [[ "$BIZ_URL" == "http://127.0.0.1:28080" || "$BIZ_URL" == "http://localhost:28080" ]]; then
  ANDROID_CONTROL_BASE_URL="http://10.0.2.2:28080"
else
  ANDROID_CONTROL_BASE_URL="$BIZ_URL"
fi
