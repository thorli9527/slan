...
SHOW_LOG_DIRS="${SLAN_CURRENT_QUICK_REGRESSION_SHOW_LOG_DIRS:-0}"
...
Purpose:
  Print the latest quick client regression summary.

Optional environment variables:
  SLAN_CURRENT_QUICK_REGRESSION_RESULT_ROOT
  SLAN_CURRENT_QUICK_REGRESSION_LATEST_LINK
  SLAN_CURRENT_QUICK_REGRESSION_SUMMARY_FILE
  SLAN_CURRENT_QUICK_REGRESSION_SHOW_LOG_DIRS=1|0

Examples:
  bash scripts/current_client_quick_summary.sh
  SLAN_CURRENT_QUICK_REGRESSION_SHOW_LOG_DIRS=1 bash scripts/current_client_quick_summary.sh
EOF
  exit 0
fi

if [[ ! -f "$SUMMARY_FILE" ]]; then
  printf 'missing quick regression summary: %s\n' "$SUMMARY_FILE" >&2
