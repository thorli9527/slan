...
WEB_CONSOLE_NAME="${PROJECT}-web-console"
...
WEB_PORT="${SLAN_WEB_CONTAINER_SMOKE_PORT:-39382}"
...
cleanup() {
  docker rm -f "${BIZ_NAME}" "${WEB_CONSOLE_NAME}" "${OPS_NAME}" "${POSTGRES_NAME}" "${REDIS_NAME}" "${BIFROMQ_NAME}" "${WEB_NAME}" "${MAIN_NAME}" >/dev/null 2>&1 || true
...
docker run -d --name "${WEB_CONSOLE_NAME}" --network "${NETWORK}" --network-alias server-biz-web-console \
  -e SLAN_BIZ_ADDR=:8080 \
  -e SLAN_BIZ_ROUTE_SET=web \
  -e SLAN_SERVICE_BIZ_DB_HOST="${POSTGRES_NAME}" \
  -e SLAN_SERVICE_BIZ_DB_PORT=5432 \
  -e SLAN_SERVICE_BIZ_DB_USER=slan \
  -e SLAN_SERVICE_BIZ_DB_PASSWORD="${POSTGRES_PASSWORD}" \
...
for _ in {1..60}; do
  if smoke_wget --header "X-Slan-Internal-Token: ${WIRE_TOKEN}" \
    ...
    smoke_wget "http://${WEB_CONSOLE_NAME}:8080/healthz" >/dev/null 2>&1 &&
    smoke_wget "http://${OPS_NAME}:8080/healthz" >/dev/null 2>&1 &&
    smoke_wget "http://${WEB_NAME}/" >/dev/null 2>&1 &&
...
smoke_wget "http://${WEB_CONSOLE_NAME}:8080/healthz" >/dev/null
...
smoke_wget "http://${WEB_NAME}/api/web/devices/visible" >/dev/null
...
echo "container stack smoke passed: app=${BIZ_PORT} web=${WEB_PORT} main=${MAIN_PORT}"
