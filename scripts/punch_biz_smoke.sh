...
AUTH="$(curl_json -X POST "${BASE_URL}/api/auth/register" \
...
BIND_A="$(curl_json -X POST "${BASE_URL}/api/device/session/bind" \
  -H 'Content-Type: application/json' \
...
curl_json -X POST "${BASE_URL}/api/device/session/bind" \
  -H 'Content-Type: application/json' \
  -H "Authorization: Bearer ${USER_TOKEN}" \
...
SESSION="$(curl_json -X POST "${BASE_URL}/api/networks/${NETWORK_ID}/punch/connect-sessions" \
  -H 'Content-Type: application/json' \
...
if curl_json -X POST "${BASE_URL}/api/networks/${NETWORK_ID}/punch/connect-sessions" \
  -H 'Content-Type: application/json' \
  -H "Authorization: Bearer ${DEVICE_TOKEN_A}" \
  -H "X-Slan-Device-ID: ${DEVICE_A}" \
  -H "X-Slan-MQTT-Username: ${MQTT_USERNAME_A}" \
  -H "X-Slan-Punch-Signature: ${PUNCH_SIGNATURE_A}" \
  -d "{\"requesterNodeId\":\"node-${DEVICE_A}\",\"peerNodeId\":\"node-missing-${RUN_ID}\",\"ttlSeconds\":30}" >/tmp/slan-punch-smoke-deny.json 2>/dev/null; then
