                        ...
                        clusters: vec![RelayCluster {
                            ...
                            nodes: vec![
                                ...
                                RelayNode {
                                    ...
                                    transport: "tcp".into(),
                                    address: support::DEV_RELAY_TCP_ADDRESS.into(),
                                    priority: 20,
                                    tags: vec![],
                                },
                            ],
                ...
                clusters: vec![DerpCluster {
                    ...
                    nodes: vec![
                        DerpNodeMeta {
                            ...
                            city_name: Some("Local".into()),
                            node_id: "relay-cn-local-udp".into(),
                            host: support::DEV_RELAY_HOST.into(),
                            port: support::DEV_RELAY_UDP_PORT,
                        ...
                        DerpNodeMeta {
                            ...
                            node_id: "relay-cn-local-tcp".into(),
                            host: support::DEV_RELAY_HOST.into(),
                            port: support::DEV_RELAY_TCP_PORT,
                            transport: DerpTransport::Udp,
                            priority: 20,
                            tags: vec![],
                        },
...
impl TunnelManager for RecordingTunnelManager {
    fn establish(&self, config: &TunnelConfig) -> Result<(), String> {
        self.established.lock().unwrap().push(format!(
            "{}->{}:{}",
            config.local_virtual_ip, config.peer_virtual_ip, config.wireguard_peer.public_key
        ));
        Ok(())
    }
...
fn connect_notifies_path_manager_on_derp_fallback() {
    ...
    assert_eq!(
        events,
        vec!["failed:peer-1:timeout", "failed:peer-1:derp_fallback"]
    );
}
...
fn probe_updates_snapshot_and_disconnect_clears_last_probe() {
    ...
    assert_eq!(
        last_probe.tunnel_peer_virtual_ip.as_deref(),
        Some("100.64.0.2")
    );
...
fn probe_respects_custom_reply_timeout_override() {
    ...
    let probe = facade
        .probe_with_timeout(b"hello".to_vec(), Some(0))
        .unwrap();
    assert!(!probe.reply_observed);
    assert!(probe.reply_bytes_received.is_none());
    assert!(probe.reply_sampled_at_ms.is_none());
