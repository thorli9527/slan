...
export abstract class AppComponentState {
  ...
  protected get effectiveUserId(): string { return this.currentUserId || DEFAULT_USER_ID; }
  ...
  showDeviceGroupPickerDialog = false;
  ...
  bindingGroupDevice: DeviceRow | null = null;
  deviceGroupName = '开发部';
  ...
  get ingressRules(): SecurityRuleRow[] {
    return this.securityRules
      .filter((rule) => rule.securityGroupId === this.selectedSecurityGroupId && rule.direction === 'ingress')
      .sort((a, b) => a.priority - b.priority || (a.ruleId ?? '').localeCompare(b.ruleId ?? ''));
  }

  get egressRules(): SecurityRuleRow[] {
    return this.securityRules
      .filter((rule) => rule.securityGroupId === this.selectedSecurityGroupId && rule.direction === 'egress')
      .sort((a, b) => a.priority - b.priority || (a.ruleId ?? '').localeCompare(b.ruleId ?? ''));
  }
  ...
  get filteredBindingDevices(): DeviceRow[] {
    const query = this.bindDeviceQuery.trim().toLowerCase();
    if (!query) {
      return this.currentUserDevices;
    }
    return this.currentUserDevices.filter((device) => {
      return (device.ip || '').toLowerCase().includes(query);
    });
  }

  bindingDeviceSelected(device: DeviceRow): boolean {
    return this.deviceGroupBindingIds.includes(device.deviceId);
  }

  bindingGroupSelected(group: DeviceGroupRow): boolean {
    return this.selectedDeviceGroupIds.includes(group.groupId);
  }
  ...
  get currentDeviceBootstrapKeys(): DeviceBootstrapKeyRow[] {
    return this.deviceBootstrapKeys
      .slice()
      .sort((a, b) => b.createdAt - a.createdAt);
  }
