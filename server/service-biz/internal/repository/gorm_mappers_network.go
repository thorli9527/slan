...
func networkConfigVersionRecordFromModel(item model.NetworkConfigVersion) gormNetworkConfigVersionRecord {
	return gormNetworkConfigVersionRecord{
		NetworkID: item.NetworkID,
...
func (r gormNetworkConfigVersionRecord) model() model.NetworkConfigVersion {
	return model.NetworkConfigVersion{
		NetworkID: r.NetworkID,
		Version:   r.Version,
		Reason:    r.Reason,
		CreatedAt: r.CreatedAt,
...
func networkDeviceRecordFromModel(item model.NetworkDevice) gormNetworkDeviceRecord {
	return gormNetworkDeviceRecord{
		...
		MembershipSource:   string(item.MembershipSource),
		PresenceStatus:     string(item.PresenceStatus),
		MQTTConnected:      item.MQTTConnected,
		VirtualIP:          item.VirtualIP,
		LastSeenAt:         item.LastSeenAt,
		LastHeartbeatAt:    item.LastHeartbeatAt,
		LastRuntimeStateAt: item.LastRuntimeStateAt,
		LastEndpointAt:     item.LastEndpointAt,
		LastPathHealthAt:   item.LastPathHealthAt,
...
func (r gormNetworkDeviceRecord) model() model.NetworkDevice {
	return model.NetworkDevice{
		...
		MembershipSource:   model.NetworkMembershipSource(r.MembershipSource),
		PresenceStatus:     model.DevicePresenceStatus(r.PresenceStatus),
		MQTTConnected:      r.MQTTConnected,
		VirtualIP:          r.VirtualIP,
		LastSeenAt:         r.LastSeenAt,
		LastHeartbeatAt:    r.LastHeartbeatAt,
		LastRuntimeStateAt: r.LastRuntimeStateAt,
		LastEndpointAt:     r.LastEndpointAt,
		LastPathHealthAt:   r.LastPathHealthAt,
