...
func mqttAllowTopicAccess(cfg MQTTConfig, principal, deviceID, topic string, subscribe bool) bool {
	...
	if principal == "server" {
		if subscribe {
			return isControlUpTopic(cfg, topic) || topic == mqttServerControlUpTopic(cfg)
		}
...
func publishControlMQTT(ctx context.Context, cfg MQTTConfig, deviceID, msgType string, payload any) error {
	...
	return mqttPublishJSON(ctx, cfg, credential.ClientID+"-"+messageID, credential.Username, credential.Password, topic, controlEnvelope{
		Type:      msgType,
		MessageID: messageID,
		Payload:   payload,
	}, mqttQoSExactlyOnce)
