...
func TestValidateRelayTopologyAcceptsUdpTransport(t *testing.T) {
	relay := DefaultConfig().Relay
	relay.Countries[0].Cities[0].Clusters[0].Nodes[0].Transport = " UDP "

	if err := validateRelayTopology(&relay); err != nil {
		t.Fatalf("expected udp to be accepted: %v", err)
	}
	if got := relay.Countries[0].Cities[0].Clusters[0].Nodes[0].Transport; got != "udp" {
		t.Fatalf("expected udp to be normalized, got %s", got)
	}
}

func TestValidateRelayTopologyRejectsUnsupportedRelayTransports(t *testing.T) {
	for _, value := range []string{"tcp", "tls", "http3", "h3", "quic"} {
		relay := DefaultConfig().Relay
