...
// ClientMessageRequest sends one lightweight control-plane message to another
// device in the same logical network.
...
type RelayNode struct {
	// NodeID 是节点 ID。
	NodeID string `json:"nodeId"`
	// Transport 是传输类型；当前 relay 数据面只支持 udp。
...
type DerpNode struct {
	...
	// Port 是节点监听端口。
	Port int `json:"port"`
	// Transport 是传输类型；DERP 数据面使用 TCP/TLS 443 兜底路径。
	Transport string `json:"transport"`
	// Priority 是控制面建议优先级。
	Priority int `json:"priority"`
	// Tags 是可选标签。
