...
func TestNormalizeOpsRelayPolicyPathTypeAcceptsCanonicalPaths(t *testing.T) {
	for _, value := range []string{"lan_udp", "ipv6_udp", "direct_udp", "relay_udp", "derp_tcp_tls_443"} {
...
func TestRelayPolicyPeerPairAddsBothTargets(t *testing.T) {
	targets := normalizeOpsRelayPolicyTargets([]string{"device-a"})
	if targets == nil {
		targets = make(map[string]bool)
	}
	addOpsRelayPolicyTarget(targets, "device-b")
	addOpsRelayPolicyTarget(targets, "device-a")

	gotTargets := sortedOpsRelayPolicyTargets(targets)
	if len(gotTargets) != 2 || gotTargets[0] != "device-a" || gotTargets[1] != "device-b" {
		t.Fatalf("unexpected targets: %#v", gotTargets)
	}
	if got := normalizeOpsRelayPolicyScope("", "device-a", "device-b", len(targets) > 0); got != "peer_pair" {
		t.Fatalf("expected peer_pair scope, got %q", got)
	}
}
