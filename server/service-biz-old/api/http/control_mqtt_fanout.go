...
func broadcastDeviceNetworkDisabled(deps routerDeps, networkID string, disabled controlmsg.DeviceNetworkDisabled) {
	if disabled.DeviceID != "" {
		_ = publishControlMQTTEnvelope(deps, disabled.DeviceID, controlmsg.DeviceNetworkDisabledEvent, "", disabled)
	}
	for _, peerSession := range mqttSessionsInNetwork(deps, networkID, "") {
		if peerSession.DeviceID == disabled.DeviceID {
			continue
		}
		_ = publishControlMQTTEnvelope(deps, peerSession.DeviceID, controlmsg.DeviceNetworkDisabledEvent, "", disabled)
	}
}
...
func publishNetworkBroadcastMQTTEnvelope(deps routerDeps, msgType string, presence controlmsg.DeviceNetworkPresence) {
	if presence.NetworkID == "" {
		return
	}
	wireMsgType := msgType
	if msgType == controlmsg.DeviceNetworkExpiredEvent {
		wireMsgType = controlmsg.DeviceNetworkDisabledEvent
	}
	...
	defer cancel()
	topic := mqttauth.NetworkBroadcastTopic(deps.Config.MQTT, presence.NetworkID)
	messageID := controlMQTTMessageID(wireMsgType)
	publisherClientID := credential.ClientID + "-network-broadcast-" + messageID
	err := mqttauth.PublishJSONWithOptions(ctx, deps.Config.MQTT, publisherClientID, credential.Username, credential.Password, topic, controlmsg.Envelope{
		Type:      wireMsgType,
		MessageID: messageID,
		Payload:   presence,
	}, mqttauth.PublishOptions{
		QoS: controlMQTTMessageQoS(wireMsgType),
	})
	if err != nil {
		log.Printf("mqtt network broadcast publish failed topic=%s type=%s sourceType=%s networkId=%s deviceId=%s err=%v", topic, wireMsgType, msgType, presence.NetworkID, presence.DeviceID, err)
		return
	}
	log.Printf("mqtt network broadcast published topic=%s type=%s sourceType=%s networkId=%s deviceId=%s qos=%d", topic, wireMsgType, msgType, presence.NetworkID, presence.DeviceID, controlMQTTMessageQoS(wireMsgType))
}
