		...
		go func() {
			for {
				...
				err := mqttauth.Subscribe(
					context.Background(),
					deps.Config.MQTT,
					credential.ClientID+"-control-up",
					credential.Username,
					credential.Password,
...
func publishControlMQTTEnvelope(deps routerDeps, deviceID, msgType, requestID string, payload any) error {
	credential := mqttauth.ServerSubscriberCredential(deps.Config.MQTT, time.Now())
	if credential == nil {
		log.Printf("mqtt control down skipped deviceId=%s type=%s reason=missing_credential enabled=%v", deviceID, msgType, deps.Config.MQTT.Enabled)
		return nil
	}
	...
	defer cancel()
	topic := mqttauth.ControlDownTopic(deps.Config.MQTT, deviceID)
	messageID := controlMQTTMessageID(msgType)
	publisherClientID := credential.ClientID + "-control-down-" + messageID
	...
	if err != nil {
		log.Printf("mqtt control down publish failed deviceId=%s topic=%s type=%s messageId=%s err=%v", deviceID, topic, msgType, messageID, err)
		return err
	}
	log.Printf("mqtt control down published deviceId=%s topic=%s type=%s messageId=%s qos=%d", deviceID, topic, msgType, messageID, controlMQTTMessageQoS(msgType))
	return nil
}
