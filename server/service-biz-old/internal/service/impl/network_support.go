...
func (s *dbState) networkPeerDTO(ctx context.Context, self dto.Node, networkID string, record repo.Node, now time.Time) (dto.Peer, bool) {
	...
	if !s.hasEnabledNetworkMember(ctx, record.DeviceID, networkID) {
		return dto.Peer{}, false
	}
	_ = now
...
func (s *dbState) hasEnabledNetworkMember(ctx context.Context, deviceID, networkID string) bool {
	exists, err := s.tokens.EnabledNetworkMemberExists(ctx, deviceID, networkID)
	return err == nil && exists
}

func (s *dbState) hasFreshOnlineNetworkState(ctx context.Context, deviceID, networkID string, now time.Time) bool {
	state, ok := s.loadFreshDeviceNetworkState(ctx, deviceID, networkID)
	if !ok {
		return false
	}
	return state.ControlReachable && state.NetworkOnline && state.TunnelUp
}
