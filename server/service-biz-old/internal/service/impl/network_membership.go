...
// 否则按默认角色创建一条激活中的成员记录。
func (s *dbState) ensureMember(ctx context.Context, networkID, deviceID, userID, role string) (dto.NetworkMember, error) {
	return s.ensureMemberWithStatus(ctx, networkID, deviceID, userID, role, "active")
}

func (s *dbState) ensureMemberWithStatus(ctx context.Context, networkID, deviceID, userID, role, status string) (dto.NetworkMember, error) {
