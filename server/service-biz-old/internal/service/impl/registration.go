...
func (s dbDeviceService) Register(userID string, req dto.RegisterDeviceRequest) (dto.Device, error) {
	...
	if err := s.state.bindDeviceUser(ctx, record.DeviceID, userID, time.Now().Unix()); err != nil {
		return dto.Device{}, err
	}
...
func (s dbDeviceService) InstallRegister(req dto.RegisterDeviceRequest, clientIP string) (dto.Device, error) {
	if err := s.requireRegisterDeviceRequest(req); err != nil {
		return dto.Device{}, err
	}
	...
	ctx := context.Background()
	now := time.Now()
	if err := s.state.ensureInstallRegistrationAllowed(ctx, strings.TrimSpace(req.DeviceID), clientIP, now); err != nil {
		return dto.Device{}, err
	}
	...
	if err := s.state.recordInstallRegistration(ctx, strings.TrimSpace(req.DeviceID), clientIP, now); err != nil {
		return dto.Device{}, err
	}
...
func (s dbDeviceService) SetDeviceNetworkState(userID, deviceID, networkID string, req dto.DeviceNetworkStateRequest) (dto.DeviceNetworkState, error) {
	ctx := context.Background()
	if err := s.state.ensureDeviceLogin(ctx, userID, deviceID); err != nil {
...
func (s *dbState) ensureDeviceOwner(ctx context.Context, userID, deviceID string) error {
	return s.ensureDeviceLogin(ctx, userID, deviceID)
}

func (s *dbState) ensureDeviceLogin(ctx context.Context, userID, deviceID string) error {
	...
	if _, err := s.pg.GetDeviceUser(ctx, deviceID, userID); err != nil {
		if repo.IsNotFound(err) {
			return ErrForbidden
		}
		return err
