...
const enabledNetworkMemberKeyPrefix = "enabled_network_member:"
const enabledNetworkMemberIndexKeyPrefix = "enabled_network_member_index:"
...
func (s *RedisTokenStore) StoreEnabledNetworkMember(ctx context.Context, state DeviceNetworkState, ttl time.Duration) error {
	if ttl <= 0 {
		ttl = 45 * time.Second
	}
	encoded, err := json.Marshal(state)
	...
	if err := s.client.Set(ctx, enabledNetworkMemberKey(state.DeviceID, state.NetworkID), encoded, ttl).Err(); err != nil {
		return err
	}
	return s.client.Set(ctx, enabledNetworkMemberIndexKey(state.DeviceID, state.NetworkID), encoded, 0).Err()
}

func (s *RedisTokenStore) EnabledNetworkMemberExists(ctx context.Context, deviceID, networkID string) (bool, error) {
	count, err := s.client.Exists(ctx, enabledNetworkMemberKey(deviceID, networkID)).Result()
	if err != nil {
		return false, err
	}
	return count > 0, nil
...
func (s *RedisTokenStore) ListEnabledNetworkMembers(ctx context.Context, networkID string) ([]DeviceNetworkState, error) {
	var cursor uint64
	var out []DeviceNetworkState
	pattern := enabledNetworkMemberKeyPrefix + "*:" + networkID
	for {
		...
		if len(keys) > 0 {
			...
			for _, value := range values {
				...
				var state DeviceNetworkState
				if err := json.Unmarshal(raw, &state); err != nil {
					log.Printf("redis enabled network member decode skipped: %v", err)
					continue
				}
				out = append(out, state)
			}
...
func (s *RedisTokenStore) ListExpiredEnabledNetworkMembers(ctx context.Context) ([]DeviceNetworkState, error) {
	var cursor uint64
	var out []DeviceNetworkState
	for {
		keys, nextCursor, err := s.client.Scan(ctx, cursor, enabledNetworkMemberIndexKeyPrefix+"*", 100).Result()
		if err != nil {
			return nil, err
		}
		cursor = nextCursor
		for _, key := range keys {
			...
			var state DeviceNetworkState
			if err := json.Unmarshal(value, &state); err != nil {
				log.Printf("redis enabled network member index decode skipped: %v", err)
				continue
			}
			exists, err := s.EnabledNetworkMemberExists(ctx, state.DeviceID, state.NetworkID)
			if err != nil {
				return nil, err
			}
			if !exists {
...
func (s *RedisTokenStore) DeleteEnabledNetworkMember(ctx context.Context, deviceID, networkID string) error {
	return s.client.Del(ctx, enabledNetworkMemberKey(deviceID, networkID), enabledNetworkMemberIndexKey(deviceID, networkID)).Err()
}
...
func enabledNetworkMemberKey(deviceID, networkID string) string {
	return enabledNetworkMemberKeyPrefix + deviceID + ":" + networkID
}

func enabledNetworkMemberIndexKey(deviceID, networkID string) string {
	return enabledNetworkMemberIndexKeyPrefix + deviceID + ":" + networkID
}
