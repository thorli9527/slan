...
func parsePublish(header byte, body []byte) (parsedPublish, error) {
	...
	propertyLength, propertyBytes, err := decodeRemainingLength(body[payloadOffset:])
	if err != nil {
		return parsedPublish{}, err
	}
	payloadOffset += propertyBytes + propertyLength
	if payloadOffset > len(body) {
		return parsedPublish{}, fmt.Errorf("mqtt publish properties truncated")
	}
