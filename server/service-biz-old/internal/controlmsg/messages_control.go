...
const (
	DeviceNetworkEnabledEvent  = "device_network_enabled"
	DeviceNetworkDisabledEvent = "device_network_disabled"
	DeviceNetworkExpiredEvent  = "device_network_expired"
)
...
type PathHealthReport struct {
	...
	// ActivePath 是客户端当前实际选中的路径，例如 lan_udp / ipv6_udp / direct_udp / relay_udp / derp_tcp_tls_443。
...
type PathOption struct {
	// PathType 是路径类型，例如 lan_udp / ipv6_udp / direct_udp / relay_udp / derp_tcp_tls_443。
	PathType string `json:"pathType"`
	// Endpoint 是目标端点。
	Endpoint string `json:"endpoint"`
...
type ConnectionState struct {
	...
	PeerNodeID string `json:"peerNodeId"`
	// Path 是当前连接使用的路径类型，例如 lan_udp / ipv6_udp / direct_udp / relay_udp / derp_tcp_tls_443。
...
// DeviceNetworkPresence 用于网络广播 topic 通知设备启用/停用。
type DeviceNetworkPresence struct {
	NetworkID     string                    `json:"networkId"`
	DeviceID      string                    `json:"deviceId"`
	...
	Reason        string                    `json:"reason,omitempty"`
	OnlineDevices []DeviceNetworkPresenceIP `json:"onlineDevices,omitempty"`
}

// DeviceNetworkPresenceIP 是首次启用时随广播下发的当前已启用设备快照。
type DeviceNetworkPresenceIP struct {
	DeviceID  string `json:"deviceId"`
	VirtualIP string `json:"virtualIp,omitempty"`
...
type ControlSyncEvent struct {
	...
	// DevicePresence 是 device_network_enabled / device_network_disabled 时携带的网络广播信息。
	DevicePresence *DeviceNetworkPresence `json:"devicePresence,omitempty"`
}
