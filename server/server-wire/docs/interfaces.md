...
`server-biz` 的 `authz` 接口，并以业务控制面返回的 `networkId`、`nodeId`、
`virtualIps`、`allowedIps` 覆盖客户端自报字段。调用时必须携带
`X-Slan-Internal-Token: $SLAN_INTERNAL_WIRE_TOKEN`。

调度约束：

- DERP ticket 与 `derpCandidates` 严格基于 `server-biz /internal/wire/derp-map`。
- relay ticket 与 `relayCandidates` 严格基于 `server-biz /internal/wire/admin/relay-nodes`。
