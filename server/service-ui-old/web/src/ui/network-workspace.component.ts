...
import { Network, NetworkAssignment, NetworkDetail, NetworkMember, NetworkQualityItem } from './api-contracts';
...
@Component({
    ...
    <section class="network-admin">
      ...
      <section class="admin-card" *ngIf="detail?.ownedByCurrentUser">
        ...
        <div class="table-shell" *ngIf="assignments.length > 0; else noAssignments">
          <table>
            <thead>
              <tr>
                ...
                <th class="client-col">客户端</th>
                <th class="runtime-col">心跳/网络/设备</th>
                <th class="quality-col">链路质量</th>
            ...
            <tbody>
              <tr *ngFor="let item of assignments">
                <td class="ip-col">{{ draftIps[item.attachmentId] || item.virtualIp || '-' }}</td>
                <td class="user-col">{{ item.userEmail }}</td>
                <td class="client-col">
                ...
                <td class="runtime-col">
                  <div class="runtime-stack">
                    ...
                    <strong [class.runtime-online]="isNetworkOnline(item)" [class.runtime-offline]="!isNetworkOnline(item)">
                      {{ isNetworkOnline(item) ? '启用' : '停用' }}
                    </strong>
                    <span>/</span>
                    <strong [class.runtime-online]="isDeviceOnline(item)" [class.runtime-offline]="!isDeviceOnline(item)">
                      {{ isDeviceOnline(item) ? '启用' : '禁用' }}
                    </strong>
                  </div>
                </td>
                <td class="quality-col">
                  <div class="quality-stack" *ngIf="qualityFor(item) as quality; else noQualityForDevice">
                    <strong>{{ quality.activePath || quality.pathType || '-' }}</strong>
                    <span>RTT {{ quality.observedRttMs || '-' }} ms · 丢包 {{ packetLossLabel(quality.packetLossPpm) }}</span>
                    <span>{{ quality.sourceCountryCode || '-' }} → {{ quality.relayCountryCode || quality.peerCountryCode || '-' }}</span>
                  </div>
                </td>
                <td class="remark-col remark-text">{{ draftRemarks[item.attachmentId] || item.remark || '-' }}</td>
                <td class="row-actions edit-cell">
                  <button class="quality-row-button" type="button" (click)="openQualityDialog.emit(item.deviceId)" [disabled]="!item.deviceId">
                    质量
                  </button>
          ...
          <ng-template #noQualityForDevice>
            <span class="quality-empty">暂无质量上报</span>
          </ng-template>
        </div>
  ...
  styles: [`
    ...
    table { width: 100%; min-width: 1010px; border-collapse: collapse; table-layout: fixed; }
    ...
    .quality-row-button { width: 42px; height: 34px; padding: 0; border-radius: 6px; color: #9a4d2f; font-size: 12px; font-weight: 900; }
    .ip-col { width: 92px; min-width: 92px; max-width: 92px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .user-col { min-width: 180px; text-overflow: ellipsis; white-space: nowrap; }
    .client-col { width: 112px; min-width: 112px; max-width: 112px; }
    .runtime-col { width: 150px; min-width: 150px; max-width: 150px; }
    .quality-col { width: 150px; min-width: 210px; max-width: 210px; }
    .remark-col { width: 150px; min-width: 130px; max-width: 130px; }
    .edit-col, .edit-cell { width: 198px; }
    ...
    .runtime-stack { display: inline-flex; align-items: center; gap: 3px; color: #94a3b8; font-size: 13px; font-weight: 900; white-space: nowrap; }
    .runtime-stack strong { min-width: 28px; font-weight: 900; text-align: center; }
    .runtime-stack strong.runtime-online { color: #15803d; }
    .runtime-stack strong.runtime-offline { color: #b91c1c; }
    .quality-stack { display: grid; gap: 4px; color: #64748b; font-size: 12px; line-height: 1.35; }
    .quality-stack strong { color: #111827; font-size: 13px; }
    .quality-stack strong.runtime-online { color: #15803d; }
    .quality-stack strong.runtime-offline { color: #b45309; }
    .quality-empty { color: #94a3b8; font-size: 12px; font-weight: 800; }
...
export class NetworkWorkspaceComponent {
  ...
  @Input({ required: true }) qualityRows!: NetworkQualityItem[];
  @Input({ required: true }) qualitySelectedDevices!: Record<string, boolean>;
  ...
  @Output() readonly toggleQualityDevice = new EventEmitter<{ deviceId: string; selected: boolean }>();
  @Output() readonly openQualityDialog = new EventEmitter<string>();
  ...
  qualityFor(item: NetworkAssignment): NetworkQualityItem | null {
    const deviceId = item.deviceId || '';
    return this.qualityRows.find((quality) => quality.deviceId === deviceId && !this.isPolicyOnlyQuality(quality)) || null;
  }

  isPolicyOnlyQuality(row: NetworkQualityItem): boolean {
    return row.pathType === 'policy' && !row.sampledAtMs;
  }
