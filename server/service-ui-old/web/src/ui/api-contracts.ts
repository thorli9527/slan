...
export type DeviceNetworkState = {
  ...
  relayPolicyScope?: string;
  relayPolicyApplied?: boolean;
  relayPolicyUpdatedAtMs?: number;
  relayPolicyAppliedAtMs?: number;
  relayMtu?: number;
  maxFramePayload?: number;
  relayPolicyReason?: string;
};
...
export type RelayPolicyTemplate = {
  templateId: string;
  networkId?: string;
  scope: 'global' | 'network' | string;
  name: string;
  description?: string;
  pathType?: string;
  pathStrategy: string;
...
export type RelayPolicyTemplateRequest = Omit<RelayPolicyTemplate, 'templateId' | 'networkId' | 'scope' | 'isBuiltin' | 'createdAt' | 'updatedAt'>;
