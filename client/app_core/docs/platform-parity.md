...
This matrix tracks how each host platform maps to the Windows app-core behavior.
...
| Control-plane RPCs | Windows service JSON bridge | Rust helper / TCP helper | Rust helper / TCP helper | JSON-line TCP bridge when a foreground service exposes `dev.slan.app_core.SERVICE_HOST` | Not implemented |
...
| Network enable / disable | Wintun service | PacketTunnel or helper-host | Linux WireGuard backend | Pending VpnService | Pending Network Extension |
...
- Android: implement a foreground app-core service that owns MQTT, XML task persistence, heartbeat, and `VpnService`; expose its JSON-line control endpoint through manifest meta-data `dev.slan.app_core.SERVICE_HOST`.
- iOS: add a Runner iOS target, Network Extension target, app group storage for queued control tasks, and a bridge between the app and extension.
