...
impl<P: PlatformNetwork> ClientRuntime<P> {
    ...
    pub fn apply_deactivation_state(&mut self) -> ClientViewState {
        self.state = ClientViewState::default();
        self.state.notice = Some("inactive".to_string());
        self.state.clone()
    }
    ...
    pub fn dispatch(&mut self, command: ClientCommand) -> Result<ClientViewState> {
        self.state.error = None;
        match command {
            ClientCommand::ApplyDeviceSession(payload) => {
                ...
                self.state.notice = Some("activated".to_string());
            }
            ...
            ClientCommand::ApplyPlatformRuntimeState(runtime_state) => {
                ...
                self.state.virtual_ip = if runtime_state.network_enabled {
                    runtime_state
                        .virtual_ip
                        .and_then(|value| normalize_virtual_ip(&value))
                } else {
