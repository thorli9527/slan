...
mod tests {
    ...
    use client_core::{ClientCommand, ClientRuntime, ClientViewState, DeviceSessionPayload};
    use client_core_platform::PlatformNetworkImpl;
    use std::{
        sync::{
    ...
    fn actor_serializes_runtime_writes_and_publishes_snapshot() {
        ...
        actor
            ...
            .expect("dispatch activation through actor");
    ...
    fn snapshot_wait_observes_actor_update() {
        ...
        actor
            ...
            .expect("update runtime through actor");

        let snapshot = snapshots.wait_after(1, Duration::from_millis(10));
        assert_eq!(snapshot.revision, 2);
        assert_eq!(snapshot.state.notice.as_deref(), Some("inactive"));
    }
    ...
    fn conditional_command_rejects_stale_revision() {
        ...
        assert_eq!(state.notice.as_deref(), Some("inactive"));
    }
    ...
    fn caller_timeout_does_not_cancel_actor_state_transition() {
        ...
        assert_eq!(actor.snapshot().state.notice.as_deref(), Some("inactive"));
        let diagnostics = actor.diagnostics();
