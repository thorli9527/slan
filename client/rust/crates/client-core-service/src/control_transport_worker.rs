...
use client_core::{AssignedIpPayload, ClientCommand, ClientMessageNoticePayload};
...
use crate::{
    ...
    network_event_projection::{apply_network_event_projection, mark_network_projection_syncing},
...
fn connect_control_mqtt_with_retry(
    ...
    for candidate in mqtt_credentials_with_proxies(credential) {
        for attempt in 1..=3 {
            ...
            match ThinControlMqttClient::connect_with_subscription_suffix(
                ...
                Err(error) => {
                    errors.push(format!(
                        "broker {} attempt {attempt}: {error}",
                        candidate.broker_url
                    ));
...
fn mqtt_credentials_with_proxies(credential: &ThinMqttCredential) -> Vec<ThinMqttCredential> {
    ...
    broker_urls
        .into_iter()
        .map(|broker_url| ThinMqttCredential {
            broker_url,
...
fn sync_after_control_mqtt_connected(
    ...
    if !state.activated || !state.network_enabled {
...
fn ingest_downstream_publish(
    ...
    if let Some(connect_plan) = try_ingest_connect_plan(payload, runtime)? {
        ...
        if current_state.activated && current_state.network_enabled {
...
fn try_ingest_device_network_membership_changed(
    ...
    if !runtime.snapshot().state.activated {
        log_service_error(
            "client-core-service ignored preactivation device_network_membership_changed message",
        );
        return Ok(true);
    }
...
fn try_ingest_network_event(
    ...
    let current_state = runtime.snapshot().state;
    let reconfigure_required = current_state.activated
...
fn publish_outbox_message(
    ...
    for candidate in mqtt_credentials_with_proxies(&credential) {
        for attempt in 1..=3 {
            ...
            match publish_outbox_message_once(&candidate, &suffix, message, &payload) {
                Ok(()) => return Ok(()),
                Err(error) => {
                    errors.push(format!(
                        "broker {} attempt {attempt}: {error}",
...
fn mqtt_connection_matches(left: &PersistedSession, right: &PersistedSession) -> bool {
    if left.session_kind != right.session_kind || left.device_id != right.device_id {
        return false;
    }
...
mod tests {
    ...
    fn subscribes_all_session_network_topics() {
        let mut session = PersistedSession::preactivation(
            "device-1",
            Some(MqttCredential {
    ...
    fn network_membership_change_keeps_the_same_worker_connection() {
        let mut before = PersistedSession::preactivation(
            "device-1",
            Some(MqttCredential {
                broker_url: "mqtt://127.0.0.1:1883".to_string(),
                client_id: "device-1".to_string(),
    ...
    fn mqtt_credential_change_requires_a_new_connection() {
        let before = PersistedSession::preactivation(
            "device-1",
            Some(MqttCredential {
                broker_url: "mqtt://127.0.0.1:1883".to_string(),
                client_id: "device-1".to_string(),
    ...
    #[test]
    fn preactivation_membership_change_is_ignored_without_stopping_ingestion() {
        ...
        let payload = serde_json::to_vec(&serde_json::json!({
            ...
            "messageId": "membership-before-activation",
            "payload": {
                "deviceId": "device-1",
                "networkId": "network-1",
        ...
        ingest_downstream_publish(&payload, &runtime, &task_queue, &state_notifier)
            .expect("ignore preactivation membership event");

        assert!(!runtime.snapshot().state.activated);
