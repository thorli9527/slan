...
use client_core::{
    relay_path_kind_for_transport, AssignedIpPayload, ClientCommand, ClientMessageNoticePayload,
    ClientRuntime, ClientViewState, PathCandidate, PathKind, PathState, PeerPathConfig,
    PlatformAclPolicy, PlatformResolverConfig, RelayDataPlaneConfig, RelayPeerSession, RouteSpec,
    SLAN_DNS_SERVICE_IP,
};
...
use crate::{
    ...
    local_api::{
        request_correlation_id, response_with_correlation_id, ActivateDeviceRequest,
    ...
    session_store::{
        activate_device_with_key, app_data_dir, current_session_runtime_epoch,
...
fn runtime() -> &'static RuntimeActorHandle {
    RUNTIME.get_or_init(|| {
        ...
        if let Err(error) = local_stable_device_id() {
            eprintln!("initialize embedded device identity: {error:#}");
        }
        let mut runtime = ClientRuntime::new(PlatformNetworkImpl);
        if let Some(session) = load_valid_registered_session() {
            let _ = runtime.dispatch(ClientCommand::ApplyDeviceSession(session.into()));
        }
...
pub fn embedded_initialize(state_dir: Option<&str>) -> Result<Value> {
    if let Some(state_dir) = state_dir.map(str::trim).filter(|value| !value.is_empty()) {
        set_app_data_dir_override(state_dir);
        set_platform_log_dir(state_dir);
    }
    let device_id = local_stable_device_id().context("initialize embedded device identity")?;
    let first_use = local_device_is_first_use()?;
    let _ = runtime();
    Ok(serde_json::json!({
        "deviceId": device_id,
        "firstUse": first_use,
...
fn embedded_relay_candidates_response(refresh: bool) -> Result<RelayCandidateListResponse> {
    ...
    runtime()
        .call_named_if_revision(
            ...
            move |runtime| {
                commit_embedded_prepared_activation(
                    runtime,
                    &prepared,
...
fn handle_request_json(request_json: &str) -> Result<String> {
    ...
    let response = match LocalServiceMethod::parse(&request.method) {
        ...
        LocalServiceMethod::LocalActivateDevice => {
            let input: ActivateDeviceRequest =
            ...
            let mqtt_session = session.clone();
            let state = runtime().call_named(
                "embedded.device.activate",
            ...
            connect_embedded_control_mqtt_with_session(&mqtt_session)
                .context("connect mqtt after device activation")?;
...
fn commit_embedded_prepared_activation(
    ...
    runtime
        ...
        .context("apply embedded prepared activation")
}
...
fn embedded_control_status() -> Value {
    match load_session() {
        Ok(session) => {
            ...
            if !network_ready && session.session_kind != "preactivation" {
...
#[cfg(test)]
mod tests {
    use client_core::{ClientCommand, DeviceSessionPayload, NetworkRuntimeState, PathKind};
    use serde_json::Value;
    ...
    fn local_state_returns_client_state() {
        ...
        assert!(response.get("activated").and_then(Value::as_bool).is_some());
        assert!(response
            .get("networkEnabled")
    ...
    fn embedded_active_network_reconcile_emits_business_event_when_network_is_enabled() {
        ...
        super::runtime()
            .call(|runtime| {
                runtime.dispatch(ClientCommand::DeactivateDevice)?;
                runtime
                    .dispatch(ClientCommand::ApplyDeviceSession(DeviceSessionPayload {
                        device_id: Some("device-1".to_string()),
                        virtual_ip: Some("10.0.0.99".to_string()),
                    }))
                    .expect("seed embedded activated runtime");
                runtime
