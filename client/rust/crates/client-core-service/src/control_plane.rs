...
use client_core::{normalize_virtual_ip, RelayTicket, RouteSpec};
...
pub(crate) fn effective_control_base_url() -> String {
...
pub(crate) fn normalize_control_base_url(value: &str) -> Result<String> {
    ...
    if normalized.is_empty() {
        bail!("server API base URL is required");
    }
...
fn resolve_control_base_url(override_value: Option<String>, env_value: Option<String>) -> String {
    override_value
        .or_else(|| {
            env_value
...
impl ControlPlaneClient {
    ...
    pub fn exchange_device_credential(&self, key: &str) -> Result<DeviceSessionResponse> {
        let key = key.trim();
        if key.is_empty() {
            bail!("授权 Key 不能为空");
        }
        ...
        let response = self.request_json_without_auth(
            "POST",
            API_DEVICE_AUTH_TOKEN,
            Some(device_credential_exchange_body(key, &device_id)),
        )?;
...
fn device_credential_exchange_body(key: &str, device_id: &str) -> Value {
    serde_json::json!({
        "key": key,
        "deviceId": device_id,
...
fn required_string(response: &Value, field: &str) -> Result<String> {
    optional_string(response, field)
        .ok_or_else(|| anyhow::anyhow!("activation response missing {field}"))
}
...
impl HttpEndpoint {
    fn parse(base_url: &str) -> Result<Self> {
        ...
        let rest = base_url.strip_prefix("http://").ok_or_else(|| {
            anyhow::anyhow!("only http control base url is supported in client bootstrap")
        })?;
...
mod tests {
    ...
    #[test]
    fn control_base_url_uses_override_then_environment_then_default() {
        assert_eq!(
            resolve_control_base_url(
                Some("http://override:8080".to_string()),
                Some("http://environment:8080".to_string()),
            ),
            "http://override:8080"
        );
        assert_eq!(
            resolve_control_base_url(None, Some(" http://environment:8080/ ".to_string())),
            "http://environment:8080"
        );
        assert_eq!(
            resolve_control_base_url(None, Some("  ".to_string())),
            DEFAULT_CONTROL_BASE_URL
        );
    }

    #[test]
    fn server_api_base_url_is_normalized_and_validated() {
        assert_eq!(
            normalize_control_base_url(" api.example.test:28080/ ").unwrap(),
            "http://api.example.test:28080"
        );
        assert!(normalize_control_base_url("https://api.example.test").is_err());
        assert!(normalize_control_base_url("   ").is_err());
    }

    #[test]
    fn device_credential_exchange_reports_platform_and_client_version() {
        let body = device_credential_exchange_body("key_secret", "device-1");
        assert_eq!(body["key"], "key_secret");
        assert_eq!(body["deviceId"], "device-1");
        assert_eq!(body["platform"], std::env::consts::OS);
        assert_eq!(body["deviceVersion"], env!("CARGO_PKG_VERSION"));
    }
