...
use crate::control_plane::{
    effective_control_base_url, local_stable_device_id, normalize_control_base_url,
    reset_local_device_id, set_control_base_url_override, ControlPeer, ControlPlaneClient,
...
use crate::local_api::{
    ...
    ServiceRequest, SetRelayTransportAllowlistRequest, WatchBusinessEventRequest,
...
use crate::session_store::{
    activate_device_with_key, app_data_dir, clear_pending_device_activation,
    current_session_runtime_epoch, current_timestamp_ms, ensure_session_node_binding,
...
fn run_service_server() -> Result<()> {
    ...
    apply_persisted_server_api_base_url()?;
    let device_id = local_stable_device_id().context("initialize local device identity")?;
    log_service_error(format!(
        "client-core-service local device identity ready: deviceId={device_id}"
    ));
    ...
    if let Some(session) = startup_session.filter(runtime_session_ready) {
        ...
        {
            log_service_error(format!(
                "client-core-service pending device activation failed: {error}"
            ));
        }
    }
    #[cfg(target_os = "windows")]
...
fn apply_pending_device_activation<P>(runtime: &mut ClientRuntime<P>) -> Option<ClientViewState>
where
    P: client_core::PlatformNetwork,
{
    let pending = load_pending_device_activation()?;
    if let Some(base_url) = pending.base_url.as_deref() {
        crate::control_plane::set_control_base_url_override(base_url);
    }
    let session = match activate_device_with_key(&pending.authorization_key) {
        Ok(session) => session,
        Err(error) => return Some(state_with_error(runtime.state(), error.to_string())),
    };
    let activation_state = runtime
        .dispatch(ClientCommand::ApplyDeviceSession(session.into()))
    ...
    if final_state.error.is_none() {
        if let Err(error) = clear_pending_device_activation() {
            log_service_error(format!(
                "client-core-service clear pending device activation failed: {error:#}"
            ));
        }
    }
    Some(final_state)
...
fn spawn_startup_network_activation(
    ...
    let should_activate = auto_activate_installed_device && runtime.snapshot().state.activated;
    if !should_activate {
        return;
    }
    thread::spawn(move || {
...
fn route_request(line: &str, context: &LocalServiceContext) -> Result<String> {
    ...
    match method {
        ...
        LocalServiceMethod::LocalServerApiSettings => return handle_local_server_api_settings(),
        LocalServiceMethod::LocalUpdateServerApiSettings => {
            let response = handle_local_update_server_api_settings(request)?;
            control_transport_worker::wake_control_transport_worker(
                &context.runtime,
                &context.task_queue,
        ...
        LocalServiceMethod::Dispatch => {
            ...
            let should_wake_control_mqtt = command_type == Some("applyDeviceSession");
        ...
        LocalServiceMethod::LocalActivateDevice => {
            return handle_activate_device(request, &context.runtime)
        }
...
fn handle_local_resolver_state() -> Result<String> {
    ...
    serde_json::to_string(&serde_json::json!({
        ...
        "desiredActivated": server.desired_activated,
    ...
    let response = match session {
        Some(session) => {
            ...
            LocalSessionResponse {
                activated: !session.access_token.trim().is_empty(),
...
fn apply_persisted_server_api_base_url() -> Result<()> {
    let device_id = local_stable_device_id().context("load device id for server API settings")?;
    let configured = crate::client_config::load_secret::<String>(
        &device_id,
        crate::client_config::KEY_SERVER_API_BASE_URL,
    )?;
    if let Some(configured) = configured {
        let normalized = normalize_control_base_url(&configured)?;
        set_control_base_url_override(&normalized);
    }
    Ok(())
}

fn handle_local_server_api_settings() -> Result<String> {
    serde_json::to_string(&serde_json::json!({
        "serverApiBaseUrl": effective_control_base_url(),
    }))
    .context("encode server API settings")
}

fn handle_local_update_server_api_settings(request: ServiceRequest) -> Result<String> {
    let value = request
        .args
        .get("serverApiBaseUrl")
        .and_then(Value::as_str)
        .context("serverApiBaseUrl is required")?;
    let normalized = normalize_control_base_url(value)?;
    let device_id = local_stable_device_id().context("load device id for server API settings")?;
    ...
    set_control_base_url_override(&normalized);
    serde_json::to_string(&serde_json::json!({
        "serverApiBaseUrl": normalized,
    }))
    .context("encode updated server API settings")
}
...
fn prepare_runtime_refresh(activated: bool) -> Result<Option<NetworkRuntimeState>> {
    if !activated {
        return Ok(None);
    }
    ...
    P: client_core::PlatformNetwork,
{
    ...
    let preserve_enabled_network = current.activated
...
fn commit_runtime_refresh(
    ...
    let expected_activated = snapshot.activated;
    let prepared = prepare_runtime_refresh(snapshot.activated);
    runtime.call_named(command_kind, correlation_id, move |runtime| {
        if !runtime_activation_context_matches(
            runtime,
...
fn handle_start(
    ...
    let prepared_runtime = prepare_runtime_refresh(registered_session_ready || snapshot.activated);
        ...
        if runtime_activation_context_matches(
            ...
            match prepared {
                Some(session) if runtime_session_ready(&session) => {
                    runtime.dispatch(ClientCommand::ApplyDeviceSession(session.into()))?;
                }
                None => {
                    runtime.apply_deactivation_state();
                }
                Some(_) => {}
            }
        } else {
...
fn runtime_activation_context_matches<P>(
...
where
    P: client_core::PlatformNetwork,
{
    runtime.state().activated == expected_activated
    ...
    P: client_core::PlatformNetwork,
{
    ...
    runtime
        .dispatch(ClientCommand::ApplyDeviceSession(
...
fn prepare_device_deactivation_remote() {
    deactivate_control_network();
}

fn commit_device_deactivation<P>(
    runtime: &mut ClientRuntime<P>,
    ...
    P: client_core::PlatformNetwork,
{
    if !runtime_activation_context_matches(
        ...
        log_service_error("client-core-service ignored stale device deactivation result");
...
fn handle_local_device_deactivation(
    runtime: &RuntimeActorHandle,
    command_kind: impl Into<String>,
    correlation_id: Option<String>,
) -> Result<ClientViewState> {
    ...
    let expected_activated = snapshot.state.activated;
    let snapshots = runtime.snapshots();
    prepare_device_deactivation_remote();
    let command_kind = command_kind.into();
    if let Err(error) = platform_transition::run_serialized_correlated(
        ...
        move |platform| {
            if snapshots.latest().revision != expected_revision {
                anyhow::bail!("stale platform device deactivation");
            }
            platform_transition::disable_network(platform)
        },
    ) {
        log_service_error(format!(
            "client-core-service device deactivation platform disable failed: {error:#}"
        ));
    }
    if runtime.snapshot().revision != expected_revision {
        return Ok(runtime.snapshot().state);
...
fn handle_dispatch_request(
    ...
    match command {
        ...
        ClientCommand::DeactivateDevice => {
            let state = handle_local_device_deactivation(
                runtime,
                "local.dispatch.device_deactivation",
                correlation_id,
            )?;
...
fn handle_activate_device(request: ServiceRequest, runtime: &RuntimeActorHandle) -> Result<String> {
    ...
    let state = runtime.call_named(
        ...
        move |runtime| {
            runtime
                .dispatch(ClientCommand::ApplyDeviceSession(session.into()))
                .context("commit activated device")
        },
    )?;
    serde_json::to_string(&state).context("encode activated device state")
}
    ...
    P: client_core::PlatformNetwork,
{
    let command = match command {
        ClientCommand::ApplyDeviceSession(_)
        | ClientCommand::EnableNetwork
        | ClientCommand::DisableNetwork
...
fn load_network_session() -> Result<PersistedSession> {
    let session = load_session().map_err(|error| {
        if session_not_found_error(&error) {
            anyhow::anyhow!("device activation required")
        } else {
            error
        }
    })?;
    if session.access_token.trim().is_empty() {
        return Err(anyhow::anyhow!("device activation required"));
    }
    Ok(session)
}
    ...
    P: client_core::PlatformNetwork,
{
    ...
    let current_device_id = runtime
        ...
        .map(str::trim)
        .filter(|value| !value.is_empty());
    if !runtime.state().activated || current_device_id != Some(planned_device_id) {
...
fn deactivate_control_network() {
    ...
    match client.deactivate_network(session_device_api_token(&session), device_id, network_id) {
        Ok(()) => log_service_error(format!(
            "client-core-service deactivated control network on device deactivation/disable: deviceId={device_id} networkId={network_id}"
        )),
        Err(error) => log_service_error(format!(
            "client-core-service failed to deactivate control network on device deactivation/disable: {error:#}"
        )),
    }
}
...
fn method_business_event_type(
    ...
    match method {
        ...
        LocalServiceMethod::Dispatch => match command_type.unwrap_or_default() {
            ...
            "applyDeviceSession" | "deactivateDevice" => BUSINESS_SESSION_CHANGED,
                ...
                Err(error) => {
                    ...
                    }
                    {
                        let state = invalidate_runtime_session(
                            &runtime,
                            "session.refresh.device_deactivation",
                            expected_snapshot.revision,
                            expected_snapshot.state.device_id,
                            expected_snapshot.state.activated,
                        )
                        .ok()
                        .flatten()
                        .unwrap_or_else(|| runtime.snapshot().state);
...
fn refresh_logged_in_session() -> Result<Option<PreparedSession>> {
    ...
    match prepare_session_device_registered(session) {
        Ok(prepared) => Ok(Some(prepared)),
        Err(error) if session_auth_invalid_error(&error) => {
            let recovered = recover_session_with_stored_authorization_key()
                .context("recover invalid background session with stored authorization")?;
            Ok(recovered.map(PreparedSession::from_session))
        }
        Err(error) => Err(error),
    }
}
...
fn spawn_runtime_sync_worker(runtime: RuntimeActorHandle, state_notifier: Arc<RuntimeEventHub>) {
    thread::spawn(move || loop {
        ...
        // network. Explicit disable flows (device deactivation) handle
        ...
        if state != before {
            let business_type = if state.activated != before.activated {
                BUSINESS_SESSION_CHANGED
    ...
    thread::spawn(move || {
        ...
        loop {
            ...
            if last_desired.as_ref() != Some(&desired) {
                log_service_error(format!(
                    "client-core-service local resolver supervisor desired changed enabled={} activated={} networkEnabled={} hasRequester={} hasResolverData={} bind={}",
                    desired.enabled,
                    desired.activated,
            ...
            if !desired.enabled {
                ...
                set_local_resolver_server_status(LocalResolverServerStatus {
                    ...
                    desired_activated: Some(desired.activated),
...

            {
                ...
                status.desired_activated = Some(desired.activated);
    ...
    thread::spawn(move || loop {
        ...
        if require_ui_refresh || after.revision != before.revision {
            let business_type = if state.error.is_some() {
                BUSINESS_NETWORK_SWITCH_FAILED
            } else if state.activated != before.state.activated {
                BUSINESS_SESSION_CHANGED
...
fn maintain_relay_data_plane(
    ...
    let before = runtime.snapshot().state;
    if !before.activated || !before.network_enabled {
        return Ok(());
    }
...
pub(crate) fn sync_control_assignment(runtime: &RuntimeActorHandle) {
    ...
    let mut session =
        match prepare_and_commit_assignment_session(runtime, "assignment.session.refresh", session)
        {
            ...
            Err(error) if session_auth_invalid_error(&error) => {
                let recovered = recover_session_with_stored_authorization_key()
                    .and_then(|session| session.context("stored authorization is unavailable"))
                    .and_then(|session| {
                        prepare_and_commit_assignment_session(
                            runtime,
                            "assignment.session.recover",
                            session,
                        )
                    });
                match recovered {
                    ...
                    Err(recovery_error) => {
                        log_service_error(format!(
                        "client-core-service assignment session recovery failed: {recovery_error:#}"
                    ));
                        let _ = invalidate_runtime_session(
                            runtime,
                            "assignment.invalid_session.device_deactivation",
                            expected_snapshot.revision,
                            expected_snapshot.state.device_id,
                            expected_snapshot.state.activated,
                        );
            ...
            Err(error) => {
                log_service_error(format!(
                    "client-core-service assignment session refresh skipped: {error:#}"
                ));
                return;
            }
        };
    ...
    if session.device_id.as_deref().is_none_or(str::is_empty) {
        let _ =
