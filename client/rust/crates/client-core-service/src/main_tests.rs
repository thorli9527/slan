use super::{
    ...
    commit_control_network_activation, commit_device_deactivation, commit_network_deactivation,
    connect_plan_content_matches, connectivity_recovery_allowed, data_plane_relay_candidate,
    diagnostic_connect_plan_summaries, filter_relay_sessions_for_transport,
...
fn device_session(device_id: &str, virtual_ip: Option<&str>) -> client_core::DeviceSessionPayload {
    client_core::DeviceSessionPayload {
...
fn stale_network_activation_plan_cannot_restore_logged_out_runtime() {
    ...
    assert!(!runtime.state().activated);
...
fn stale_network_activation_result_preserves_current_runtime_state() {
    ...
    let mut runtime = ClientRuntime::new(TestPlatformNetwork);
    runtime
        .dispatch(ClientCommand::ApplyDeviceSession(device_session(
            "new-device",
            Some("10.0.0.9"),
        )))
        .expect("apply current activation");
...
#[test]
fn stale_invalid_session_result_cannot_deactivate_new_runtime_session() {
    let runtime = RuntimeActorHandle::spawn(ClientRuntime::new(PlatformNetworkImpl));
    let stale_revision = runtime.snapshot().revision;
    runtime
        .call_named("test.activation", None, |runtime| {
            runtime.dispatch(ClientCommand::ApplyDeviceSession(device_session(
                "new-device",
                None,
            )))
        })
    ...
    let invalidated = invalidate_runtime_session(
        &runtime,
        "test.stale.device_deactivation",
        stale_revision,
        None,
        false,
    )
...
#[test]
fn stale_deactivation_cannot_clear_current_runtime_session() {
    let mut runtime = ClientRuntime::new(TestPlatformNetwork);
    runtime
        .dispatch(ClientCommand::ApplyDeviceSession(device_session(
            "new-device",
    ...
    let state =
        commit_device_deactivation(&mut runtime, Some("old-device".to_string()), true, false);
