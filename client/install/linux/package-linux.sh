...
install_dir="$repo_root/client/install/linux"
. "$install_dir/lib/slan-linux-install.sh"

flutter_build_dir=""
service_bin="$repo_root/client/rust/target/release/client-core-service"
output_dir="$repo_root/client/.tmp/installer/linux"
variant="all"
...
if [ -z "$flutter_build_dir" ]; then
  flutter_build_dir="$repo_root/client/app_flutter/build/linux/$(slan_linux_flutter_arch_dir "$arch")/release/bundle"
fi
