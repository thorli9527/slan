...
This document describes the target iOS MQTT flow. Legacy native Swift MQTT/device activation/control-plane code is intentionally out of scope.

## Ownership

- `client-core-ffi` exposes the embedded `client-core-service` JSON API to Flutter/iOS.
- Embedded `client-core-service` owns device activation, session persistence, MQTT credentials, MQTT5 control transport, downstream policy consumption, and upstream status publishing.
...
## Device Activation And Session

1. Opt signs an authorization Key for a device.
2. Mobile UI sends the Key through embedded `localActivateDevice`.
3. Embedded service exchanges the Key for a device token through `ControlPlaneClient`.
4. Embedded service persists the device session and MQTT credential.
5. Flutter connects embedded `localConnectControlMqtt`.
6. Flutter reads current state from embedded `localState` or embedded watch responses.

## Device Registration And MQTT Credential

1. Device registration is owned by embedded service, not Swift.
2. After activation or activated startup, Flutter calls embedded `localEnsureDevice`.
...
3. Events originate from:
   - device activation/session changes
   - IP reassignment
...
For iOS, any new MQTT/control-plane work must go through embedded `client-core-service` via `client-core-ffi`. Swift plugin code must not implement device activation, MQTT subscribe/publish, downstream control parsing, or policy selection.
