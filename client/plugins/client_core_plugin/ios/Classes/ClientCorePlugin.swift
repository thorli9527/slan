...
@_silgen_name("client_core_v2_initialize")
private func clientCoreV2Initialize(
...
public class ClientCorePlugin: NSObject, FlutterPlugin {
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    ...
    let initialized = plugin.initializeEmbeddedRuntime()
    plugin.state["deviceId"] = initialized["deviceId"]
    plugin.state["firstUse"] = initialized["firstUse"] ?? true
    if let error = initialized["error"] {
  ...
  private func initializeEmbeddedRuntime() -> [String: Any] {
#if SLAN_CLIENT_CORE_FFI
    let stateDir = FileManager.default.urls(
      for: .applicationSupportDirectory,
      in: .userDomainMask
    ).first?.path ?? ""
    guard let responsePointer = stateDir.withCString({ clientCoreV2Initialize($0) }) else {
      return ["firstUse": true, "error": "embedded runtime initialization returned null"]
    }
    defer { clientCoreV2FreeString(responsePointer) }
    let response = String(cString: responsePointer)
    guard let data = response.data(using: .utf8),
      let object = try? JSONSerialization.jsonObject(with: data) as? [String: Any]
    else {
      return ["firstUse": true, "error": "embedded runtime initialization returned invalid json"]
    }
    return object
#else
    return ["firstUse": true, "error": "ios client-core-service embedded FFI is not linked yet"]
#endif
  }
