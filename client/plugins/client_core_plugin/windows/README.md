...
The Windows plugin must not implement mesh, adapter, DNS, route, MQTT, or device activation business logic. It only forwards UI commands to the local service and returns `ClientViewState`.
...
- The session stores the device token, device id, active network id, and assigned virtual IP.
...
- Run it after device activation and network enable to validate the local data plane:
