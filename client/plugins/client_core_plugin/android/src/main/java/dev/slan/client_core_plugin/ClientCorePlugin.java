...
public final class ClientCorePlugin
    ...
    synchronized (stateLock) {
      try {
        JSONObject initialized =
            new JSONObject(SlanNativeBridge.initialize(applicationContext.getFilesDir().getAbsolutePath()));
        String deviceId = initialized.optString("deviceId", "").trim();
        state.put("deviceId", deviceId.isEmpty() ? null : deviceId);
        state.put("firstUse", initialized.optBoolean("firstUse", true));
        String error = initialized.optString("error", "").trim();
        if (!error.isEmpty()) {
          state.put("error", error);
        }
      } catch (Exception error) {
        state.put("deviceId", null);
        state.put("firstUse", true);
        state.put("error", "initialize client runtime: " + error.getMessage());
      }
  ...
  private void resetState(boolean inactive) {
    synchronized (stateLock) {
      state.clear();
      state.put("activated", false);
      state.put("deviceId", null);
      state.put("nodeId", null);
      state.put("networkEnabled", false);
      state.put("syncing", false);
      state.put("switchEnabled", true);
      state.put("notice", inactive ? "inactive" : "androidClientReady");
