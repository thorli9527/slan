...
`client` is the current client architecture for SLAN. It keeps Flutter thin and moves durable client business logic into Rust.

## Boundary

Flutter owns:

- Device activation with an Opt-issued authorization Key
- Current device IP and network enabled/syncing/error UI
- Enable/disable intent from the switch

Rust `client-core-service` owns:

- Device identity, activation, heartbeat, and device session state
...
The tray icon changes between network enabled and disabled/inactive states.
...
The service stores MQTT credentials after exchanging an authorization Key for a device token. Flutter does not read or manage these credentials.
