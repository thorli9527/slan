...
- The `Network` switch item is disabled until the local service reports an activated device.
- The icon itself distinguishes network state: active uses the enabled icon; inactive, deactivated, and service-unavailable states use the disabled icon.
...
- The status icon tooltip follows the same model as the Flutter UI: it listens to `localStateWatch` and only updates when the service state revision changes.
...
- `Open` brings the main Flutter window forward. The `Network` switch item sends the local service command only when the device is activated.
...
- Install: `scripts/install_macos_service.sh --app client/app_flutter/build/macos/Build/Products/Release/slan_client_v2.app`
