...
constexpr const wchar_t kTrayTooltipInactive[] =
...
bool IsTrayNetworkActionEnabled(const TrayServiceState& state) {
  return state.activated && state.switch_enabled && !state.syncing;
}
...
TrayServiceState QueryTrayServiceState() {
  ...
  state.activated = JsonBoolField(response, "activated");
...
void ToggleNetworkFromTray() {
  const TrayServiceState state = QueryTrayServiceState();
  if (!state.activated || state.syncing || !state.switch_enabled) {
    return;
  }
